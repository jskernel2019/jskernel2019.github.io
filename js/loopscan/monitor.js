...
var options = {
    title: 'Max loopscan gaps for Different URLs',
    vAxis: {title:"URL"},
    hAxis: {title:"Max Gap (ms)"},
    width: 900,
    height: 500,
    bar: {groupWidth: "95%"},
...
var draw_data = [['URL','Max Gap']];

function drawChart() {
    console.log(draw_data);
    var view = new google.visualization.DataView(google.visualization.arrayToDataTable(draw_data));
    ...
    function summarize(raw) {
        ...
        if (!qrTotal) {
            ...
            if(draw_data.length == 1){
                draw_data.push(['youtube.com',qrTotal["max"]]);
            }
            else{
                draw_data.push(['google.com',qrTotal["max"]]);
                drawChart();
            }
